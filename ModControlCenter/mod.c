...
static void init_mod(kernel_mod_handle_t* handle) {
    mod_logger_write(
        MOD_LOG_LEVEL_INFO,
        "ModControlCenter",
        "MOD Control Center v0.1.3 initializing (native eglSwapBuffers test, author: liuxin)"
    );
    ...
    mod_logger_write(
        MOD_LOG_LEVEL_INFO,
        "ModControlCenter",
        "MOD Control Center v0.1.3 initialized"
    );
}
...
static void cleanup_mod(kernel_mod_handle_t* handle) {
    ...
    mod_logger_write(
        MOD_LOG_LEVEL_INFO,
        "ModControlCenter",
        "MOD Control Center v0.1.3 unloaded"
    );
}
